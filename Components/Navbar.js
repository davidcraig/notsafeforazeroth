...
function renderNavigationItem(item, onNavigate, openByDepth, setOpenByDepth, depth = 0) {
  ...
  return <Link className={itemCssClass} key={item.slug} title={item.name} href={item.slug} onClick={onNavigate}>{item.name}</Link>
}
  ...
  return (
    <nav className={`navbar p-4 flex flex-col gap-4 md:flex-row ${props.className}`}>
      <div className='flex items-center w-full md:w-auto justify-between md:justify-start'>
        <a className='brand font-bold text-xl shrink-0 mr-4' href='/'>&lt;Not Safe for Azeroth&gt;</a>
